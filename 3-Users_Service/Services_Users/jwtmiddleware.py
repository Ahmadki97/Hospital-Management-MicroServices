...
from Helper_Users.redis import redis_client
...
class JwtMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if not authorization or not authorization['access_token']:
        ...
        try:
            ...
            print(f"Username is: {request.state.user['username']}")
            if redis_client.exists(token):
                print(f'Token is in the blacklist')
                raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="User is loggedOut, please login again to get new token")
