...
from Helper_Auth.redis import redis_client
...
async def logoutUser(db: Session, token: str):
    try:
        ...
        print(f'payload in logoutuser() method is: {payload}')
        id = payload.get('id')
        print(f"ID of the user is: {id}")
        redis_client.setex(name=token, value="BlackListed", time=604800)
        logger.info(f"Token for user with id {id} deleted successfully.")
    except Exception as err:
