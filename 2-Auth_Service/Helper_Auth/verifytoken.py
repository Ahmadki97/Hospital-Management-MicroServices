...
def jwt_required(func):
    ...
    async def decoratedFunction(request: Request, *args, **kwargs):
        ...
        if 'Authorization' not in request.headers:
            raise NotAuthorizedError('Request Not Authorized', 'jwt_required(): There is no Authorization Header, Request not coming from API Gateway')
        api_header = request.headers.get('Authorization')
        if not api_header.startswith('Bearer '):
            raise NotAuthorizedError('Request Not Authorized', 'jwt_required(): There is no Bearer in Header, Request not coming from API Gateway')
        try:
