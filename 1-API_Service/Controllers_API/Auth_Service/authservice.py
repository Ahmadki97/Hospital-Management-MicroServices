...
async def signupAdmin(request: Request):
    ...
    try:
        ...
        return RedirectResponse(url="/api/v1/auth/login/admin", status_code=status.HTTP_303_SEE_OTHER)
    except Exception as err:
...
async def signupPatient(request: Request):
    ...
    try:
        ...
        return RedirectResponse(url="/api/v1/auth/login/patient", status_code=status.HTTP_303_SEE_OTHER)
    except Exception as err:
...
async def loginAdmin(request: Request):
    try:
        token = await request_handler.makeRequest(endpoint='login/admin', service_token='auth', request=request)
...
async def loginPatient(request: Request):
    try:
        token = await request_handler.makeRequest(endpoint='login/patient', service_token='auth', request=request)
...
@auth_router.get("/login/logout")
async def logout(request: Request):
    try:
        await request_handler.makeRequest(endpoint='login/logout', service_token='auth', request=request)
        logger.info(f"logout() Controller, Request sent to the Aplicable Service")
        return {"message": "User logged out successfully"}
    except Exception as err:
        logger.error(f"Error in logout() Method: {err}")
        return {"message": "Error in logout() process."}